(function() {
    ...
    function customList() {
        return {
            ...
            template: '' +
                ...
                '<md-list-item class="md-2-line " ng-repeat="item in customListCtrl.items | filter: customListCtrl.filterFunction | filter: customListCtrl.filterName | filter: customListCtrl.filterDate"  >' +
                '<div class="el md-padding">' +
                '<div ng-click="customListCtrl.toggleSelection(item)">' +
                '<md-checkbox ng-checked="item.checked" class="md-primary box_ck">' +
                '</md-checkbox>' +
                '</div>'+
                '<div class="box_priority" ><md-button ng-click="customListCtrl.changePriority(item)" class="md-icon-button box_priority" aria-label="Priority">' +
                ...
                '</md-button></div> ' +
                '<div class="box1 i"><md-button aria-label="Info" ng-click="customListCtrl.ShowDescription(item.description,item.name)" />info</md-button></div>' +
                '<div class="box1 xx"><div class="md-list-item-text"' +
                '<h3><b>{{item.name}}</b></h3></div>' +
                '<p>Expiration date:{{item.exp_date | date: "dd/MM/yyyy"}}</p>' +
                '</div>' +
                '<div class="box_state"><md-input-container>'+
                '<label>Change State in</label>'+
                '<md-select ng-model="select_model" >'+ 
                '<md-option ng-if="item.done" ng-click="customListCtrl.change_to_todo(item)" layout-align="center center">TODO</md-option>'+
                '<md-option ng-if="!item.done" ng-click="customListCtrl.change_to_done(item)" layout-align="center center">DONE</md-option>'+
                '</md-select></div>'+
    ...
    function customListController($scope,storageService, $mdDialog) {
        var vm = this;
        vm.IsVisible = false;
        var item;
